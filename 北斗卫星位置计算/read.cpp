...
void readeph(parameter s[1375],string broadcastfile)
{
	//读取数据
	...
	string buf;//临时存储数据的每一行
	int realnumber = 0;//实际的文件总行数
	int number = 0;//开始读文件体的小循环，因为GPS和BDS卫星星历一个结构有8行，这个用于判断到了哪一行
	...
	//进入读文件体程序
	for (realnumber; realnumber < 87055; )//87055之后没有BDSW卫星
	{
		getline(file, buf);//每一次取一行的内容
		realnumber++;//实际的总行数加一
		if (realnumber < 79224)
			...
			number = (realnumber - 79223) % 8;//开始读文件体的小循环，因为一个结构有8行，这个用于判断到了哪一行，到了70463之后就开始是C开头的卫星了
		//先判断开头，只读入C和G开头的卫星
	...
	for (int i = Gnumber; i < 1375; i++)
	{
		...
		tem.min = (int)s[i].min;
		tem.sec = s[i].sec;
		BDSTime temp = MJD2BDSTime(CalendarTime2MJD(tem));
	...
	for (int i = Gnumber; i < 1375; i++)
	{
		...
		cout << "该卫星在读取的北斗卫星中的序列号为：" << s[i].broadcastnum << endl;
