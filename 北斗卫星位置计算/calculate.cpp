...
double calculate_x(BDSTime dstBDST, parameter ep)
{
	...
	ofstream ofs07;
	ofstream ofs21;
	ofs01.open("D:/北斗卫星位置计算/北斗卫星位置数据/BDS卫星C01轨道平面坐标.txt",ios_base::app);
	ofs07.open("D:/北斗卫星位置计算/北斗卫星位置数据/BDS卫星C07轨道平面坐标.txt",ios_base::app);
	ofs21.open("D:/北斗卫星位置计算/北斗卫星位置数据/BDS卫星C21轨道平面坐标.txt",ios_base::app);
	if(parameter::count>=397&&parameter::count<421)
	{
		ofs01.flags(ios::fixed);
		ofs01.precision(6);//设置保留小数位数，精确到mm，取6位
		ofs01 << xpk <<" "<< ypk <<endl;
	}
	if(parameter::count>=553&&parameter::count<577)
	{
		ofs07.flags(ios::fixed);
		ofs07.precision(6);//设置保留小数位数，精确到mm，取6位
		ofs07 << xpk <<" "<< ypk <<endl;
	}
	if(parameter::count>=864&&parameter::count<888)
	{
		ofs21.flags(ios::fixed);
		ofs21.precision(6);//设置保留小数位数，精确到mm，取6位
		ofs21 << xpk <<" "<< ypk <<endl;
	}
	ofs01.close();
	ofs07.close();
